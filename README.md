...
ドメインモデルの設計（ddd-model）またはドメイン知識の取り込み（domain-expert）→ DDD 観点のレビュー（ddd-review）→ 役割分担での開発サイクル（ddd-orchestrate）が連携して動きます。
...
| スキル | 起動 | 説明 |
|--------|------|------|
| [ddd-model](ddd-model/SKILL.md) | 自動 | 要件からこれから作るドメインモデルを設計するモデリングスキル。イベントストーミング流のヒアリング（ドメインイベント→コマンド→ルール）で要件を業務の言葉のまま整理し、集約境界・不変条件・ユビキタス言語・コンテキスト境界を設計して domain-expert 互換のメモリに永続化する。 |
